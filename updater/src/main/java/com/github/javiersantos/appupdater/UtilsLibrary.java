...
class UtilsLibrary {
	...
	static String getAppInstalledVersion(Context context) {
		...
		try {
			version = context.getPackageManager().getPackageInfo("com.wallview.app", 0).versionName;
	...
	static Integer getAppInstalledVersionCode(Context context) {
		...
		try {
			versionCode = context.getPackageManager().getPackageInfo("com.wallview.app", 0).versionCode;
		} catch (PackageManager.NameNotFoundException e) {
			e.printStackTrace();
		}
